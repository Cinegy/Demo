...
/*------------------------------------------------------*/
	var visualPart = function(command) {
		...
		if (command != 'Start') $('.left-wrap section div').attr('class','off');
		...
		switch (command) {
			case 'Black':
				...
				break;
			case 'Bypass':
				$('#bypass').attr('class','on');
				break;
			case 'Clean':
				$('#clear').attr('class','on');
				break;
			case 'Normal':
				$('#normal').attr('class','on');
				break;
			default:
				console.log( 'Miss' );
		}
		...
		for(var j=0; listArr.length > j; j++){
			...
			if (iterCount == 0 ) {
				...
				var newElem = '<div class="item" data-count="'+j+'" data-bind="'+listArr[j]._Id.replace(/}|{/g,'')+'"><span>'+
					listArr[j]._Name+'</span><span class=""><i></i></span><span>'+
					listArr[j]._Duration+'</span><span>'+
					listArr[j]._ScheduledAt+'</span></div>';
				...
				if ( loop == 'y' ) {
					...
					if ( loopNext == undefined ) {
						var loopClass = 'loop-end';
					}
				}
